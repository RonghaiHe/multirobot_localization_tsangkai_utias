...
class LocalStateBlockDiagonalApproximation():
    ...
    def prediction(self, t):
        ...
        for r in range(NUM_ROBOTS):
            ...
            W[1, 1] = 0.0 # because 2nd input is not included
            Q = G @ W @ G.T
        ...
        for r in range(NUM_ROBOTS):
            ...
            for idx in idxs:
                ...
                if barcode_num in barcode_nums:
                    ...
                    H = u.rot_mtx(theta).T @ (-1.0*np.identity(2))
                    ...
                    self.robot_system.xys[2*r:2*r+2] += Kalman_gain @ (z - z_hat)
                    self.robot_system.covs[2*r:2*r+2, 2*r:2*r+2] -= Kalman_gain @ H @ cov
                    ...
                    for rr in range(NUM_ROBOTS):
                        if not rr == r:
                            self.robot_system.covs[2*r:2*r+2, 2*rr:2*rr+2] = multi_i @ self.robot_system.covs[2*r:2*r+2, 2*rr:2*rr+2]
                            self.robot_system.covs[2*rr:2*rr+2, 2*r:2*r+2] = self.robot_system.covs[2*rr:2*rr+2, 2*r:2*r+2] @ multi_i.T
        ...
        for r in range(NUM_ROBOTS):
            ...
            for idx in idxs:
                ...
                if barcode_num in barcode_nums:
                    ...
                    # block diagonal approximation
                    covs = self.robot_system.covs
                    reduced_covs = np.zeros(shape=covs.shape)
                    reduced_covs[2*r:2*r+2, 2*r:2*r+2] = covs[2*r:2*r+2, 2*r:2*r+2]
                    reduced_covs[2*r_j:2*r_j+2, 2*r_j:2*r_j+2] = covs[2*r_j:2*r_j+2, 2*r_j:2*r_j+2]
                    reduced_covs[2*r:2*r+2, 2*r_j:2*r_j+2] = covs[2*r:2*r+2, 2*r_j:2*r_j+2]
                    reduced_covs[2*r_j:2*r_j+2, 2*r:2*r+2] = covs[2*r_j:2*r_j+2, 2*r:2*r+2]

                    multi_i = np.linalg.inv( covs[2*r:2*r+2, 2*r:2*r+2] )
                    multi_j = np.linalg.inv( covs[2*r_j:2*r_j+2, 2*r_j:2*r_j+2] )
                    ...
                    innovation = H @ reduced_covs @ H.T + R
                    Kalman_gain = reduced_covs @ H.T @ np.linalg.inv(innovation)

                    self.robot_system.xys[2*r:2*r+2] += Kalman_gain[2*r:2*r+2] @ (z - z_hat)
                    self.robot_system.xys[2*r_j:2*r_j+2] += Kalman_gain[2*r_j:2*r_j+2] @ (z - z_hat)

                    self.robot_system.covs -= Kalman_gain @ H @ reduced_covs

                    # block diagonal approximation
                    multi_i = covs[2*r:2*r+2, 2*r:2*r+2] @ multi_i
                    multi_j = covs[2*r_j:2*r_j+2, 2*r_j:2*r_j+2] @ multi_j

                    for rr in range(NUM_ROBOTS):
                        if not rr == r and rr == r_j:
                            self.robot_system.covs[2*r:2*r+2, 2*rr:2*rr+2] = multi_i @ self.robot_system.covs[2*r:2*r+2, 2*rr:2*rr+2]
                            self.robot_system.covs[2*r_j:2*r_j+2, 2*rr:2*rr+2] = multi_j @ self.robot_system.covs[2*r_j:2*r_j+2, 2*rr:2*rr+2]

                    for rr in range(NUM_ROBOTS):
                        if not rr == r and rr == r_j:
                            self.robot_system.covs[2*rr:2*rr+2, 2*r:2*r+2] = self.robot_system.covs[2*rr:2*rr+2, 2*r:2*r+2] @ multi_i.T
                            self.robot_system.covs[2*rr:2*rr+2, 2*r_j:2*r_j+2] = self.robot_system.covs[2*rr:2*rr+2, 2*r_j:2*r_j+2] @ multi_j.T
