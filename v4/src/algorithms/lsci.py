...
class LocalStateCovarianceIntersection():
        ...
        for r in range(NUM_ROBOTS):
            ...
            for idx in idxs:
                ...
                if barcode_num in barcode_nums:
                    ...
                    V = u.rot_mtx(bearing)
                    # meas_var = np.array([[RANGE_VAR, 0.0], [0.0, BEARING_VAR]]) # origin
        ...
        for r in range(NUM_ROBOTS):
            ...
            for idx in idxs:
                ...
                if barcode_num in barcode_nums:
                    ...
                    # predicted xy, different from v3
                    xy_j = np.matrix([u.rot_mtx(theta) @ z + self.robot_system.xys[2*r:2*r+2]]).T # np.array((x, y))

                    # robot covs
                    sigma_z = u.rot_mtx(bearing) @ np.matrix([[RANGE_VAR, 0], [0, _range**2*BEARING_VAR]]) @ u.rot_mtx(bearing).T
                    cov = self.robot_system.covs[2*r:2*r+2, 2*r:2*r+2] + u.rot_mtx(theta) @ sigma_z @ u.rot_mtx(theta).T
                    
                    cov_j = self.robot_system.covs[2*r_j:2*r_j+2, 2*r_j:2*r_j+2]
                    # print([np.linalg.det(cov), np.linalg.det(cov_j)])
                    # if(np.linalg.det(cov)<1e-5 or np.linalg.det(cov_j)<1e-5): continue
                    # ls-ci weight
                    e = 0.83

                    # relative observation update
                    cov_j_next = e*np.linalg.inv(cov_j) + (1.-e)*np.linalg.inv(cov)

                    self.robot_system.xys[2*r_j:2*r_j+2] =(np.linalg.inv(cov_j_next) @ \
                                                           ( e*np.linalg.inv(cov_j) @ np.matrix([self.robot_system.xys[2*r_j:2*r_j+2]]).T  + (1.-e)*np.linalg.inv(cov) @ xy_j )).T[0]

                    self.robot_system.covs[2*r_j:2*r_j+2, 2*r_j:2*r_j+2] = np.linalg.inv(cov_j_next)

                    # print([np.linalg.inv(cov_j), np.linalg.inv(cov), self.robot_system.covs[2*r_j:2*r_j+2, 2*r_j:2*r_j+2]])
                    # print([r, r_j])
                    # print([x_j, y_j, xy_j, self.robot_system.xys[2*r_j], self.robot_system.xys[2*r_j+1]])
