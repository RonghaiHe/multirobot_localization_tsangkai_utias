...
class GlobalStateCovarianceIntersection():
    ...
    def prediction(self, t):
        ...
        for r in range(NUM_ROBOTS):
            ...
            for rr in range(NUM_ROBOTS):
                ...
                covs = self.robot_system.robots[r].covs

                if rr == r:
                    covs[2*rr:2*rr+2, 2*rr:2*rr+2] += Q

                else:
                    covs[2*rr:2*rr+2, 2*rr:2*rr+2] += W

                self.robot_system.robots[r].covs = covs
                self.robot_system.robots[r].covs = covs
        ...
        for r in range(NUM_ROBOTS):
            ...
            for idx in idxs:
                ...
                if barcode_num in barcode_nums:
                    ...
                    innovation = H @ covs @ H.T + R
                    Kalman_gain = covs @ H.T @ np.linalg.inv(innovation)

                    self.robot_system.robots[r].xys += Kalman_gain @ (z - z_hat)
                    self.robot_system.robots[r].covs -= Kalman_gain @ H @ covs
        ...
        for r in range(NUM_ROBOTS):
            ...
            for idx in idxs:
                ...
                if barcode_num in barcode_nums:
                    ...
                    V = u.rot_mtx(bearing)
                    meas_var = np.array([[RANGE_VAR, 0.0], [0.0, _range**2 * BEARING_VAR]]) # add!
                    R = V @ meas_var @ V.T
            ...
            for s in range(NUM_ROBOTS):
                ...
                for r in range(NUM_ROBOTS):
                    ...
                    if s != r:
                        ...
                        # sender information
                        s_covs = self.robot_system.robots[s].covs

                        # receiver information
                        r_covs = self.robot_system.robots[r].covs

                        covs_inv = e*np.linalg.inv(s_covs) + (1.-e)*np.linalg.inv(r_covs)

                        # update the sender's state
                        self.robot_system.robots[s].xys = np.linalg.inv(covs_inv) @ ( (e*np.linalg.inv(s_covs)) @ self.robot_system.robots[s].xys \
                                                            + (1.-e)*np.linalg.inv(r_covs) @ self.robot_system.robots[r].xys )

                        # update the sender's covariance
                        self.robot_system.robots[s].covs = np.linalg.inv(covs_inv)
    ...
    def save_est(self, t):
        ...
        for r in range(NUM_ROBOTS):
            ...
            y = self.robot_system.robots[r].xys[2*r+1]
            theta = self.robot_system.robots[r].thetas[r]
            cov = self.robot_system.robots[r].covs[2*r:2*r+2, 2*r:2*r+2]
